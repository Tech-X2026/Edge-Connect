  ...
  return (
    <section id="digital-marketing" className="relative overflow-hidden bg-white py-20 md:py-28">
      ...
      <div ref={ref} className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          {/* Header */}
          <motion.div variants={fadeInRight} className="mb-12 text-right md:text-right">
            <span className="mb-4 inline-block rounded-full border border-cyan-200 bg-cyan-50/80 px-4 py-1.5 text-xs font-semibold tracking-wider uppercase text-cyan-700">
              Digital Marketing
            </span>
            ...
            <p className="ml-auto max-w-2xl text-base leading-relaxed text-ec-muted sm:text-lg">
            ...
            {channels.map((channel) => {
              ...
              return (
                <motion.div
                  ...
                  className="group rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all duration-300 hover:border-cyan-100 hover:shadow-lg hover:shadow-cyan-50/50 sm:p-8"
                >
                  <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-50 to-cyan-100/50">
                    <Icon className="h-6 w-6 text-cyan-700" />
                  </div>
                  <h4 className="mb-2 text-lg font-bold text-gray-900 sm:text-xl">{channel.title}</h4>
                  <p className="text-sm leading-relaxed text-ec-muted sm:text-base">{channel.description}</p>
                </motion.div>
              )
            })}
          </motion.div>
          ...
          <div className="grid gap-10 lg:grid-cols-2">
            {/* Stats */}
            <motion.div variants={fadeInLeft} className="rounded-2xl bg-gradient-to-br from-cyan-600 to-cyan-700 p-8 md:p-10">
              ...
              <div className="grid grid-cols-2 gap-6">
                {stats.map((stat) => (
                  <div key={stat.label}>
                    ...
                    <div className="mt-1 text-sm text-cyan-100">{stat.label}</div>
                  </div>
                ))}
              </div>
              <motion.button
